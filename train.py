...
from src.models import UNet, DualTimeUNet, create_model_from_config
from src.data import create_dataloader_from_config, save_image, unnormalize
from src.methods import DDPM_V2, DDPM, FlowMapMatching
...
def save_checkpoint(
    ...
    # Save method state (for annealing schedule, etc.)
    if method is not None:
        checkpoint['method'] = method.state_dict()
...
def load_checkpoint(
    ...
    # Load method state (for annealing schedule, etc.)
...
def generate_samples(
    ...
    **sampling_kwargs,
    ...
    samples = None
    # Determine sampling steps: allow override via sampling_kwargs
    ...
    while remaining > 0:
        b = min(remaining, config.get('sampling', {}).get('batch_size', num_samples))
        # call method.sample; pass num_steps if available
        if num_steps is not None:
            batch_samples = method.sample(batch_size=b, image_shape=image_shape, num_steps=num_steps, **sampling_kwargs)
        else:
            batch_samples = method.sample(batch_size=b, image_shape=image_shape, **sampling_kwargs)

        samples_list.append(batch_samples.detach().cpu())
        remaining -= b

    samples = torch.cat(samples_list, dim=0)[:num_samples]
...
def save_samples(
    ...
    # samples may be (C,H,W) or (B,C,H,W)
    if samples.dim() == 3:
    ...
    if images.shape[0] == 1:
        ...
        # Create grid with default nrow ~= sqrt(N)
        n = images.shape[0]
...
def train(
    ...
    # For flow_map_matching: wrap base UNet in DualTimeUNet BEFORE DDP
    # This ensures DDP is the outermost wrapper
    if method_name == 'flow_map_matching':
        if is_main_process:
            print("Wrapping base model in DualTimeUNet for dual-time conditioning...")
        # Wrap base model for dual-time input (BEFORE DDP)
        model = DualTimeUNet(base_model, num_timesteps=config.get('flow_map_matching', {}).get('num_timesteps', 1000)).to(device)
    else:
        model = base_model

    # Apply DDP wrapping if distributed (DDP must be outermost)
    if is_distributed:
    ...
    # Count parameters (use base_model for consistent reporting)
    num_params = sum(p.numel() for p in base_model.parameters())
    if is_main_process:
        print(f"Base model parameters: {num_params:,} ({num_params / 1e6:.2f}M)")
        if method_name == 'flow_map_matching':
            # DualTimeUNet adds time_s_embedding parameters
            total_params = sum(p.numel() for p in unwrap_model(model).parameters())
            print(f"Total parameters (with DualTimeUNet): {total_params:,} ({total_params / 1e6:.2f}M)")

    # Create method instance with the properly wrapped model
    if method_name == 'flow_map_matching':
        method = FlowMapMatching.from_config(model, config, device)
    elif method_name == 'ddpm':
        method = DDPM.from_config(model, config, device)
    elif method_name == 'ddpm_v2':
        method = DDPM_V2.from_config(model, config, device)
    else:
        raise ValueError(f"Unknown method: {method_name}. Supported methods: 'ddpm', 'ddpm_v2', 'flow_map_matching'.")

    # Create optimizer from method.model (ensures all trainable params are included)
    # For flow_map_matching, this includes DualTimeUNet's time_s_embedding
    optimizer = create_optimizer(method.model, config)
    ...
    if resume_path is not None:
        ...
        if is_distributed:
            dist.barrier()
        start_step = load_checkpoint(resume_path, method.model, optimizer, ema, scaler, device, method)
    ...
    for step in pbar:
        ...
        # Gradient clipping (use method.model to ensure all params are clipped)
        if gradient_clip_norm > 0:
            scaler.unscale_(optimizer)
            torch.nn.utils.clip_grad_norm_(method.model.parameters(), gradient_clip_norm)
        ...
        if (step + 1) % save_every == 0:
            if is_main_process:
                ...
                save_checkpoint(checkpoint_path, method.model, optimizer, ema, scaler, step + 1, config, method)
    ...
    if is_main_process:
        ...
        save_checkpoint(final_path, method.model, optimizer, ema, scaler, num_iterations, config, method)
...
def main():
    ...
    parser.add_argument('--method', type=str, required=True,
                       choices=['ddpm', 'ddpm_v2', 'flow_map_matching'],
                       help='Method to train: ddpm, ddpm_v2, or flow_map_matching')
